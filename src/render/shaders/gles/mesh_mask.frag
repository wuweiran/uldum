...
// GLES port of mesh_mask.frag — same math as mesh.frag but with the
// glTF alphaMode=MASK discard so cut-out geometry (leaves, decals,
// fences) renders the right silhouette instead of its bounding box.
...
void main() {
    ...
    // glTF alphaMode=MASK: discard pixels below the cutoff.
