...
private:
    ...
    // Client-side ambient-loop handle map. The authoritative script assigns a
    // logical handle; each client maps it to its own AudioEngine handle so the
    // matching S_AMBIENT_STOP stops the right loop.
