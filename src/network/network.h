...
public:
    ...
    bool init_offline();
    bool init_host(u16 port, u32 max_players, GameServer& server);
    bool init_client(std::string_view address, u16 port, GameClient& client);
    ...
    // On clients, set_hud_message_fn() installs a callback that gets each
    ...
    void send_to_peer(u32 peer_id, std::span<const u8> packet, bool reliable = true);
    ...
    u32 connected_peer_id(u32 index) const { return m_peers[index].peer_id; }
    simulation::Player connected_peer_player(u32 index) const {
        return m_peers[index].player;
    }
