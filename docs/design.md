...
### Phase 27 — Editor Asset Manager

A map maker building a real game has no way to see or vet the assets their map depends on. Models, textures, and sounds are just path strings buried in the map's data files; the only way to know whether a model loads, faces the right way, is the right size, or has the animation clips the engine expects is to place it in the world and eyeball it. There is no view of the map's files, no thumbnail, no "play this sound," and no feedback on how the engine actually reads a given asset. Phase 27 makes the editor an asset workbench: a file explorer over the map package where the maker can see everything the map contains, organize it, preview each asset, and read what the engine's loader makes of it.

**Panel visibility — View menu.** The editor's windows become toggleable from a new **View** menu. Scenes, Tools, and Info show by default; the Asset Manager is available but hidden by default, opened from View when the maker wants it. Each entry is a simple show/hide toggle so the workspace can be tailored to the task at hand.

**Phase 27a — Map file explorer.** The core of the feature: a real file explorer over the open map package, mirroring exactly where each file lives on disk. Because makers are free to place assets at any path inside the map, the explorer shows the map's own folder structure rather than sorting everything into engine-imposed buckets — it is the map's file tree, browsable the way the maker organized it. From here the maker can manage the package directly: import new source files into a chosen folder (converting to the engine's runtime formats where needed), and rename, move, and delete files in place. Because map data references assets by path, the explorer warns when an operation would break an existing reference (deleting or renaming a file the map still points at).

**Phase 27b — Asset preview.** Selecting a file previews it in the way that suits its kind: a model appears in the viewport so the maker can orbit and inspect it as the engine would render it; a texture shows as an image; a sound plays on demand. Preview is read-only inspection — it never alters the map or its placements.

**Phase 27c — Asset info (read-only).** Alongside the preview, an info panel reports what the engine's loader sees when it reads the asset — the same checks the engine already runs at load time, surfaced here instead of being buried in a log. For a model that means its factual makeup (meshes, vertices, bones, animation clips, materials) plus any warnings the loader raises about content that won't behave as authored. The info panel only *reports* — it states what is and what the engine flagged; it does not prescribe fixes or offer to modify the asset. This turns the engine's existing model validation into visible, at-a-glance feedback for the maker.

**Later — candidates, each its own scope call.** Thumbnail previews so the explorer shows assets at a glance rather than one-at-a-time; richer read-only detail (texture resolution/detail, sound length); browsing the read-only engine assets a map inherits alongside its own; and detecting unused files and dangling references across the whole map.
