...
### Phase 27 — Editor Map Explorer

A map maker building a real game has no way to see or vet the assets their map depends on. Models, textures, and sounds are just path strings buried in the map's data files; there is no view of the map's files, no "play this sound," and no feedback on how the engine reads a given asset. Phase 27 makes the editor an asset workbench, reached through a new **View** menu that toggles the editor's windows (Scene Switcher, Tool Panel, Info Panel show by default; the Map Explorer is hidden until opened).
