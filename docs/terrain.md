...
One diffuse texture per layer, provided by the map as KTX2 (authored from PNG via the editor's Import PNG). Tiles once per terrain tile (128 game units). The engine generates procedural fallback textures if files are missing.
