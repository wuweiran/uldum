...
**Output:** `build\bin\` — `uldum_dev.exe`, `uldum_editor.exe`, `uldum_worker.exe`, `uldum_server.exe`, `uldum_pack.exe`, plus `engine.uldpak` and every `maps\*.uldmap\` from the engine repo packed into `build\bin\maps\`. **No `uldum_game.exe`** — that target only appears in per-project game builds (below).
